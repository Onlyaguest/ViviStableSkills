...
async function subscribeRtmChannelWithRetry(
  rtmClient: RTMClient,
  channel: string,
  maxAttempts = 6,
  delayMs = 2000,
  ...
  throw lastError instanceof Error ? lastError : new Error('Failed to subscribe RTM channel')
}
...
export function useAgoraConnection() {
  ...
  useEffect(() => {
    ...
    const initializeSession = async () => {
      try {
        ...
        await subscribeRtmChannelWithRetry(rtmClient, config.channel)
  ...
  const renewAgoraTokens = useCallback(async () => {
    ...
    try {
      // The backend returns a combined RTC+RTM token for the same account identity.
      const nextConfig = await getConfig({ channel: config.channel, uid: String(config.uid) })
      await client.renewToken(nextConfig.token)
      await rtmClientRef.current?.renewToken(nextConfig.token)
    } catch (nextError) {
