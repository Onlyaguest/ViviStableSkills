...
from agora_agent.agentkit.agent_session import StartAgentsRequestProperties, resolve_session_presets
...
class Agent:
    ...
    async def _start_session_with_raw_response(self, session: Any) -> str:
        """
        Start the session and read agent_id from raw response.

        Workaround for SDK response parsing where session.start() may return ""
        even when the remote start succeeds.
        """
        if session._status not in ("idle", "stopped", "error"):
            raise RuntimeError(f"Cannot start session in {session._status} state")

        session._validate_avatar_config()
        session._status = "starting"

        try:
            if session._token:
                token_opts = {"token": session._token}
            else:
                token_opts = {
                    "app_id": session._app_id,
                    "app_certificate": session._app_certificate,
                    "expires_in": session._expires_in,
                }

            properties = session._build_start_properties(token_opts)
            resolved_preset, resolved_properties = resolve_session_presets(session._preset, properties)

            try:
                request_properties: Any = StartAgentsRequestProperties(**resolved_properties)
            except Exception:
                request_properties = resolved_properties

            raw_response = await session._client.agents.with_raw_response.start(
                session._app_id,
                name=session._name,
                properties=request_properties,
                preset=resolved_preset,
                pipeline_id=session._pipeline_id,
                request_options=session._request_options(),
            )
            response_json = raw_response._response.json()
            agent_id = response_json.get("agent_id")
            if not agent_id:
                raise RuntimeError(f"Missing agent_id in start response: {response_json}")

            session._agent_id = agent_id
            session._status = "running"
            session._emit("started", {"agent_id": agent_id})
            return agent_id
    ...
    async def start(
        ...
        try:
            agent_id = await self._start_session_with_raw_response(session)
        except Exception:
            logger.exception(
